...
export class RentalsService {
  ...
  getRentals(filter: RentalsFilter): Observable<Page<Rental>> {
    ...
    return this.httpClient.get<Page<Rental>>(`${this.apiUrl}/rentals`, {
      params: httpParams,
    });
  }
  ...
  getCars(filter: CarsFilter): Observable<Page<Car>> {
    const httpParams = toHttpParams(filter);
    return this.httpClient.get<Page<Car>>(`${this.apiUrl}/cars`, {
      params: httpParams,
    });
  }

  addCar(request: AddCarRequest): Observable<AddCarResponse> {
    return this.httpClient.put<AddCarResponse>(`${this.apiUrl}/cars`, request)
  }
  ...
  getDictionaries(): Observable<Dictionary[]> {
    if (!this.dictionaries$) {
      this.dictionaries$ = this.httpClient
        .get<Dictionary[]>(`${this.apiUrl}/dictionaries`)
        .pipe(
          share({
